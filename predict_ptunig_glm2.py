...
def set_args():
    ...
    parser.add_argument('--model_dir',
                        default="./output-glm2-ptuning/epoch-2-step-180/", type=str,
                        help='')
...
def main():
    ...
    with open(args.test_path, "r", encoding="utf-8") as fh:
        for i, line in enumerate(tqdm(fh, desc="iter")):
            with torch.no_grad():
                ...
                tokens = ["[gMASK]", "<sop>"] + prompt_tokens + src_tokens
    ...
    print(f1 / 50)
    save_path = os.path.join(args.model_dir, "ft_pt_answer_ptuning.json")
